...
> Auto-maintained by OpenWolf. Last scanned: 2026-07-18T18:54:29.954Z
...
- `verify-install.js` — root: rel, fileExists, parseJson + 11 more (~4740 tok)
...
- `index.ts` — Exports createProgram (~2264 tok)
- `init.ts` — Exports initCommand (~9818 tok)
...
- `update.ts` — openwolf update — Update all registered OpenWolf projects. (~6022 tok)

## src/config/

- `default-config.json` (~1868 tok)
...
- `stop.ts` — Atomically claim a per-session nudge slot for the given counter field. (~26241 tok)
...
- `daemon-cmd.test.js` — Declares dir (~925 tok)
