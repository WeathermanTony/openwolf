...
function buildRelease(checkout: string, commit: string, skills: SkillLock[]): { path: string; key: string } {
  ...
  try {
    ...
    if (!fs.existsSync(release)) fs.renameSync(staging, release);
      ...
      fs.rmSync(staging, { recursive: true, force: true });
      for (const skill of skills) {
        const existing = path.join(release, "marketplace", "plugins", PLUGIN_NAME, "skills", skill.id);
        if (!fs.existsSync(existing) || hashDirectory(existing) !== skill.hash) throw new Error(`Existing release hash mismatch for ${skill.id}.`);
      }
      if (!fs.existsSync(path.join(release, "release.json"))) fs.writeFileSync(path.join(release, "release.json"), JSON.stringify({ commit, skills }, null, 2) + "\n");
    }
