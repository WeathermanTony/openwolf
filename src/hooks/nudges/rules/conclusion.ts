...
export function collectCoveredHashes(qaDir) {
    ...
    for (const name of names) {
        ...
        for (const h of reductionTargetHashes(path.join(qaDir, name))) covered.add(h);
    }
    return covered;
}
...
export function collect({
    ...
    if (!text || text.length < minTextChars) return { candidates: [], matched: [] };

    const matched = [];
    for (const src of patterns) {
        try {
            if (new RegExp(src, "i").test(text)) matched.push(src);
        } catch {
            // Bad user-supplied pattern: skip it rather than crash the hook.
        }
    }
    // Signal 1 absent → nothing to do.
    if (matched.length < minHits) return { candidates: [], matched };
    ...
    for (const owner of owners.values()) {
        ...
        for (const file of owner.files) {
            ...
            if (!h) continue; // deleted/unreadable → not a reduction obligation
            targetHashes[file] = h;
            if (!covered.has(h)) uncovered.push(file);
        }
