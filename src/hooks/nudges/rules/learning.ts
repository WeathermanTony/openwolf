...
export function classifyExplicitLearning(text) {
    ...
    if (/^(?:here(?:'s| is)|quoted?|example|the (?:file|text|message|user)(?: message)? says?)\b.{0,120}(?:\b(?:i|we)\s+(?:prefer|chose|decided)\b|\b(?:don't|do not|never|always|remember to)\b|\b(?:use|do)\b.+\binstead\b)/i.test(s)) return null;
