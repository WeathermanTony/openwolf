...
export function classifyExplicitLearning(text) {
    ...
    if (s.length < 12 || s.length > 4000 || QUESTION.test(s) || CONTROL.test(s) || SECRET.test(s))
        return null;
    if (/^(?:here(?:'s| is)|quoted?|example|the (?:file|text|message|user) says?)\b.{0,80}\b(?:i|we)\s+(?:prefer|chose|decided)\b/i.test(s))
        return null;
    const patterns = [
