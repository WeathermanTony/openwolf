...
export interface RollingWindowOptions<T> {
  ...
  file: string;
  /** Key under which the array lives (e.g., "bugs", "reviews"). */
  arrayKey: string;
  /** Extract an ISO-8601 timestamp string from an entry. */
  getDate: (entry: T) => string | undefined;
  /** Entries older than this are archived. Set to <=0 or null to disable. */
  retentionDays: number | null;
  /**
   * Optional: extract a stable unique id from an entry. When provided,
   * archive merges dedup by id, which protects against duplicates if
   * the live-file trim fails after a successful archive write (the
   * stale entries would otherwise be re-archived on the next run).
   * For buglog.json pass `e => e.id`; for reviewlog pass `e => e.id`.
   */
  getId?: (entry: T) => string | undefined;
  /** Where to write archives. Defaults to <wolfDir>/archive/. */
  archiveDir?: string;
 ...
 * is out of window.
 */
export function rollingWindowJson<T>(opts: RollingWindowOptions<T>): RollingWindowResult {
  const result: RollingWindowResult = { trimmed: 0, archived: 0, archivePath: null };

  if (opts.retentionDays === null || opts.retentionDays <= 0) return result;
  if (!fs.existsSync(opts.file)) return result;
...
export interface CappedSessionsOptions<T = unknown> {
  file: string;
  arrayKey: string;
  /** Max entries kept inline. When exceeded, the oldest are archived. */
  maxInline: number;
  /**
   * Optional dedup-by-id on archive merge. For token-ledger sessions,
   * pass `e => e.sessionId` if entries have one.
   */
  getId?: (entry: T) => string | undefined;
  archiveDir?: string;
  now?: number;
 ...
 */
export function cappedSessionsJson<T = unknown>(opts: CappedSessionsOptions<T>): RollingWindowResult {
  const result: RollingWindowResult = { trimmed: 0, archived: 0, archivePath: null };

  if (opts.maxInline <= 0) return result;
  if (!fs.existsSync(opts.file)) return result;
  ...
  try {
    ...
    const arr = doc[opts.arrayKey];
    if (!Array.isArray(arr) || arr.length <= opts.maxInline) return result;

    const overflow = arr.length - opts.maxInline;
    const archive = (arr as T[]).slice(0, overflow);
    const keep = (arr as T[]).slice(overflow);
    ...
    const archiveOk = mergeIntoArchive<T>(archivePath, opts.arrayKey, archive, doc, opts.getId);
    if (!archiveOk) return result;

    doc[opts.arrayKey] = keep;
    if (!atomicWriteJson(opts.file, doc)) return result;
...
export interface MonthlyRotateOptions {
  file: string;
  /** Period in days to keep inline. Older content moves to monthly archive. */
  retentionDays: number | null;
  /**
   * Optional: a regex matching the "row separator" pattern. Lines starting
   * with this are treated as boundaries. For memory.md we use session
   * headers (## Session: YYYY-MM-DD HH:MM). When omitted, every line is
   * a row and dating is by line position rather than content.
   */
  sectionHeaderPattern?: RegExp;
  /**
   * Extract a date from a section header line. Required when
   * sectionHeaderPattern is set.
   */
  getSectionDate?: (headerLine: string) => string | undefined;
  archiveDir?: string;
  now?: number;
}
 ...
 */
export function monthlyRotateMarkdown(opts: MonthlyRotateOptions): RollingWindowResult {
  const result: RollingWindowResult = { trimmed: 0, archived: 0, archivePath: null };

  if (opts.retentionDays === null || opts.retentionDays <= 0) return result;
  if (!opts.sectionHeaderPattern || !opts.getSectionDate) return result;
  if (!fs.existsSync(opts.file)) return result;
  ...
  try {
    ...
    for (let i = 0; i < lines.length; i++) {
      if (headerRe.test(lines[i])) sectionStarts.push(i);
    }
    ...
    type Section = { lines: string[]; dateMs: number | null };
    ...
    const keepSections: Section[] = [];
    const archiveSections: Section[] = [];
    for (const sec of sections) {
      if (sec.dateMs === null || sec.dateMs >= cutoffMs) keepSections.push(sec);
      else archiveSections.push(sec);
    }

    if (archiveSections.length === 0) return result;
    ...
    const archiveOk = mergeIntoMarkdownArchive(
      archivePath,
      archiveSections.map((s) => s.lines),
      headerRe,
    );
    if (!archiveOk) return result;
    ...
    const liveText = preamble + (keepSections.length > 0
      ...
      : "\n");
    if (!atomicWriteText(opts.file, liveText)) return result;
...
export interface RotateLogOptions {
  file: string;
  /** Rotate when size exceeds this many bytes. Set to <=0 to disable. */
  maxBytes: number;
  /** Number of rotations to keep (file.1, file.2, ...). Older are deleted. */
  keep: number;
}

export interface RotateLogResult {
  rotated: boolean;
  removed: string[];
}
 ...
 */
export function rotateLogFile(opts: RotateLogOptions): RotateLogResult {
  const result: RotateLogResult = { rotated: false, removed: [] };

  if (opts.maxBytes <= 0) return result;
  if (!fs.existsSync(opts.file)) return result;
  ...
  try {
    ...
    if (stat.size < opts.maxBytes) return result;

    // Delete rotations beyond keep
    for (let n = opts.keep + 1; n <= opts.keep + 10; n++) {
      const old = `${opts.file}.${n}`;
      if (fs.existsSync(old)) {
        try { fs.unlinkSync(old); result.removed.push(old); } catch {}
      } else {
        break;
      }
    }
    ...
    if (fs.existsSync(overflow)) {
      try { fs.unlinkSync(overflow); result.removed.push(overflow); } catch {}
    }
...
function atomicWriteText(filePath: string, content: string): boolean {
  const tmp = filePath + "." + crypto.randomBytes(4).toString("hex") + ".tmp";
  try {
 ...
 */
function withoutStatefulFlags(re: RegExp): RegExp {
...
export function acquireFileLock(
  targetPath: string,
  opts: { timeoutMs?: number; staleMs?: number; absoluteStaleMs?: number } = {},
): (() => void) | null {
...
function tryReclaim(
  ...
  try {
      ...
      if (code === "EEXIST") {
        ...
        try {
          ...
          if (Date.now() - st.mtimeMs > staleMs * 2) {
            try { fs.unlinkSync(reclaimLockPath); } catch {}
          }
        } catch {}
        return false;
      }
    ...
    if (currentContent !== expectedContent) return false; // Someone reclaimed.
    if (!isLockReclaimable(lockPath, staleMs, absoluteStaleMs)) return false;
    ...
    try { fs.unlinkSync(reclaimStaging); } catch {}
  }
}
...
function isLockReclaimable(
  ...
  if (age > absoluteStaleMs) return true; // Fail-safe.
  if (age <= staleMs) return false;       // Fresh — not stale.
  ...
  if (!Number.isFinite(pid) || pid <= 0) return true; // Malformed nonce.
 ...
 */
function mergeIntoMarkdownArchive(
  archivePath: string,
  newSections: string[][],
  headerRe: RegExp,
): boolean {
 ...
 */
function splitIntoSections(content: string, headerRe: RegExp): string[][] {
...
function mergeIntoArchive<T>(
  ...
  try {
    ...
    if (fs.existsSync(archivePath)) {
      ...
      const existingEntries = Array.isArray(existingArr) ? (existingArr as T[]) : [];
...
function monthSlugUtc(ms: number): string {
  const d = new Date(ms);
 ...
 */
function safeCall<R>(fn: () => R, fallback: R): R {
  try { return fn(); } catch { return fallback; }
}

/** Stable content hash for markdown section dedup. */
function sectionHash(lines: string[]): string {
